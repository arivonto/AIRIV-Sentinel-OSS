...
import sentinel.systemd_production_probe_live_execution as module
from sentinel.systemd_production_probe_live_execution import ARGV, COMPONENT, UNIT, ProductionProbeLiveRequest, SystemdProductionProbeLiveExecution
from test_systemd_commander_integration_v1 import make_runtime, make_identity, make_snapshot


def payload():
    return dict(schema_version=1, request_id="request-1", approval_id="D8D12E-ONE-LIVE-DBUS-PROBE-RESTART",
...
def setup(tmp_path, monkeypatch):
    ...
    surface = SystemdProductionProbeLiveExecution(runtime.commander, runtime.remediation_action_catalog,
                                        root=tmp_path / "inbox", snapshot_provider=provider,
                                        clock=lambda: 1050)
    plan = ProductionProbeLiveRequest.parse(json.dumps(payload()), 1050).build_plan(before)
...
def test_cycle_absence_never_publishes_or_creates_state(setup, monkeypatch, existing_root):
    runtime, orch, fake, surface, snaps, calls, plan = setup
    if existing_root:
        surface.root.mkdir(mode=0o700)
    def readonly_write(*args, **kwargs):
        raise OSError("read-only state root")
    monkeypatch.setattr(module, "atomic_write", readonly_write)
    assert surface.cycle() is None
    assert surface.cycle() is None
    assert surface.root.exists() is existing_root
    if existing_root:
        assert list(surface.root.iterdir()) == []
    assert surface._generation == 0
    assert calls == fake.calls == []
    assert orch.identity_boundary.journal.get_live_run_permit(plan.effect.run_id) is None
    empty(runtime)


@pytest.mark.parametrize("legacy_runtime_override", [False, True])
def test_secure_default_absence_is_inert(setup, tmp_path, monkeypatch, legacy_runtime_override):
    runtime, orch, fake, injected, snaps, calls, plan = setup
    home = tmp_path / "home"
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: home))
    if legacy_runtime_override:
        monkeypatch.setenv("AIRIV_SENTINEL_RUNTIME_DIR", str(tmp_path / "legacy-runtime"))
    else:
        monkeypatch.delenv("AIRIV_SENTINEL_RUNTIME_DIR", raising=False)
    surface = SystemdProductionProbeLiveExecution(
        runtime.commander, runtime.remediation_action_catalog,
        snapshot_provider=injected.snapshot_provider,
    )
    assert surface.root == home / ".local/state/airiv-sentinel-secure/systemd_production_probe_live"
    assert surface.poll_once() is None
...
def test_explicit_root_and_filenames_preserved(setup, tmp_path):
    surface = setup[3]
    assert surface.root == tmp_path / "inbox"
    assert surface.request_path == surface.root / "systemd-canary-live-request.json"
    assert surface.state_path == surface.root / "systemd-canary-live-state.json"
    assert surface.evidence_path == surface.root / "systemd-canary-live-evidence.json"
...
def test_durable_replay_after_reconstruction(setup, crash, monkeypatch):
    ...
    retry = SystemdProductionProbeLiveExecution(runtime.commander, runtime.remediation_action_catalog,
...
def test_request_immutable(setup):
    request = ProductionProbeLiveRequest.parse(json.dumps(payload()), 1050)
...
def test_concurrent_delivery_delegates_once(setup):
    ...
    runtime, orch, fake, surface, snaps, calls, plan = setup
    other = SystemdProductionProbeLiveExecution(runtime.commander, runtime.remediation_action_catalog,
