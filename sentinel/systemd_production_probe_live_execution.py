...
D8D12E_APPROVAL_ID = "D8D12E-ONE-LIVE-DBUS-PROBE-RESTART"

@dataclass(frozen=True, slots=True)
class ProductionProbeLiveRequest:
    schema_version: int
    request_id: str
    approval_id: str
    created_at: float
    ...
    def __post_init__(self):
        if self.approval_id != D8D12E_APPROVAL_ID:
            raise ValueError("approval_not_exact_d8_12e")
...
class SystemdProductionProbeLiveExecution:
    ...
    def cycle(self):
        with self._lock:
            # poll_once owns request-state publication. An empty inbox must
            # remain inert even when the secure state root is not writable.
        ...
        with self._lock:
            ...
            try:
                ...
                if raw is None:
                    return None
                consumed = True
                request = ProductionProbeLiveRequest.parse(raw, self.clock())
